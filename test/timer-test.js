...
it("timer(callback) verifies that callback is a function", end => {
  assert.throws(function() { d3.timer(); }, TypeError);
  assert.throws(function() { d3.timer("42"); }, TypeError);
  assert.throws(function() { d3.timer(null); }, TypeError);
  end();
});
...
it.skip("timer(callback) uses the global context for the callback", end => {
  const t = d3.timer(function() {
...
it("timer(callback) invokes callbacks in scheduling order during synchronous flush", end => {
  const results = [];
  const t0 = d3.timer(function() { results.push(1); t0.stop(); });
  const t1 = d3.timer(function() { results.push(2); t1.stop(); });
  const t2 = d3.timer(function() { results.push(3); t2.stop(); });
  d3.timerFlush();
...
it("timer(callback, delay) invokes callbacks in scheduling order during asynchronous flush", end => {
  const then = d3.now();
  let results;
  const t0 = d3.timer(function() { results = [1]; t0.stop(); }, 60, then);
  const t1 = d3.timer(function() { if (results) results.push(2), t1.stop(); }, 40, then);
  const t2 = d3.timer(function() { if (results) results.push(3), t2.stop(); }, 80, then);
  const t3 = d3.timer(function() {
    t3.stop();
    assert.deepStrictEqual(results, [1, 2, 3]);
    end();
...
it("timer(callback) within a frame invokes the callback at the end of the same frame", end => {
  const then = d3.now();
  const t0 = d3.timer(function(elapsed, now) {
    const delay = d3.now() - then;
    const t1 = d3.timer(function(elapsed2, now2) {
      t1.stop();
      assert.strictEqual(elapsed2, 0);
      assert.strictEqual(now2, now);
      assertInRange(d3.now() - then, delay, delay + 3);
      end();
    }, 0, now);
    t0.stop();
  });
...
it("timer(callback, delay) within a timerFlush() does not request duplicate frames", end => {
  ...
  const t0 = d3.timer(function(elapsed) {
...
it("timer.stop() immediately stops the timer", end => {
  let count = 0;
  const t = d3.timer(function() { ++count; });
  setTimeout(function() {
    t.stop();
...
it("timer.stop() recomputes the new wake time after one frame", end => {
  ...
  const t0 = d3.timer(function() {}, 1000);
  const t1 = d3.timer(function() {}, 500);
  setTimeout0(function() {
    ...
    setTimeout0(function() {
      ...
      // 3. The alarm was reset to wake for the long-delay d3.
...
it("timer.restart(callback) implicitly uses zero delay and the current time", end => {
  const t = d3.timer(function() {}, 1000);
...
it("timer.restart(callback, delay, time) recomputes the new wake time after one frame", end => {
  ...
  const t = d3.timer(function() {}, 500, then);
  setTimeout0(function() {
...
it("timer.stop() immediately followed by d3.restart() doesn’t cause an infinite loop", end => {
  const t = d3.timer(function() {});
  let last;
  t.stop();
  t.restart(function(elapsed) {
...
it("timer.stop() immediately followed by d3.restart() doesn’t cause an infinite loop (2)", end => {
  const t0 = d3.timer(function() {}), t1 = d3.timer(function() {});
  let last;
  t0.stop();
